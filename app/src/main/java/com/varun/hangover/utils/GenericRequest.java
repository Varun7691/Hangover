...
/**
 * Created by Administrator on 12 Aug,2016.
 */
public class GenericRequest<T> extends JsonRequest<T> {
     ...
     */
    public GenericRequest(int method, String url, Class<T> classtype, String toBeSent,
                          Response.Listener<T> listener, Response.ErrorListener errorListener, Map<String, String> headers, boolean mute) {
        this(method, classtype, url, toBeSent, listener,
                errorListener, headers);
        this.muteRequest = mute;
    }

    public GenericRequest(int method, String url, Class<T> classtype, String toBeSent,
                          Response.Listener<T> listener, Response.ErrorListener errorListener, Map<String, String> headers, boolean mute, boolean isStringResponse) {
        this(method, classtype, url, toBeSent, listener,
                errorListener, headers);
        this.muteRequest = mute;
    ...
    protected Response<T> parseNetworkResponse(NetworkResponse response) {
        ...
        final long cacheHitButRefreshed = 100; // in 100 milliseconds cache will be hit, but also refreshed on background
        ...
        if (muteRequest) {
            ...
            try {
                ...
                if (isStringResponse) {
                    return (Response<T>) Response.success(json, cacheEntry);
                } else {
                    T parsedObject = gson.fromJson(json, clazz);
                    return Response.success(parsedObject, cacheEntry);
                }
